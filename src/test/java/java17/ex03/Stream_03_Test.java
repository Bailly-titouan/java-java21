...
public class Stream_03_Test {
	...
	public void test_map_collect_joining() throws Exception {
		...
		// par le caractère "|"
		String result = customers.stream().map(Customer::getFirstname).sorted().reduce((name1, name2) -> name1 + "|" + name2).get();
	...
	public void test_partitionning() throws Exception {
		...
		Map<Boolean, List<Pizza>> result = pizzas.stream().collect(Collectors.groupingBy(p -> p.getName().charAt(0) == 'L'));
