...
public class Stream_07_Test {
    ...
    Function<List<Pizza>, List<Pizza>> filterPizza = f -> f.stream().filter(p -> p.getPrice() >= 1000).toList();

    // TODO créer une fonction List<Pizza> -> List<Pizza>
    // TODO seules les pizzas ayant un prix >= 1000 sont conservées
    // TODO .parallel()
    Function<List<Pizza>, List<Pizza>> parallelFilterPizza = f -> f.stream().parallel().filter(p -> p.getPrice() >= 1000).toList();
